...
class TextAnalyzer:
    ...
    def _calculate_professional_metrics(self) -> Dict:
        ...
        if total_words > 0:
            clarity_factors = [
                (1 - metrics["passive_voice_count"] / len(self.sentences)) * 25,
                (1 - metrics["long_sentences"] / len(self.sentences)) * 25,
                (1 - metrics["complex_words"] / total_words) * 25,
                (1 - metrics["repetitive_words"] / total_words) * 25
            ]
